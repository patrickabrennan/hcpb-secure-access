...
locals {
  ...
  # Decrypt EC2 Windows password_data only when a private key PEM is provided.
  # Otherwise fall back to a provided plaintext password (decrypted_admin_password), or empty string.
  admin_password = try(
    (var.admin_key_private_pem != "" ? rsadecrypt(aws_instance.rdp_target.password_data, var.admin_key_private_pem) : null),
